...
async function main(tenantDomain?: string, tenantId?: string, policyId?: string) {
    ...
    if (!tenantDomain || !tenantId) {
        ...
        if(!tenantDomain || !tenantId)
            throw new Error("Tenant domain and id are required");
    ...
    if (policySet) {  
        const policySetPath = `./exported-userflows/${policyId}.xml`;
...
if(argv.tenantDomain)
    tenantDomain = argv.tenantDomain as string;
if(argv.tenantId)
    tenantId = argv.tenantId as string;
if(argv.policyId)
    policyId = argv.policyId as string;

main(tenantDomain, tenantId, policyId).catch((err) => {
