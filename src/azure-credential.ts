...
export class AzureCredential implements TokenCredential {
  private _options: AzureCredentialOptions & { allowAppOnly: boolean };
  ...
  async getToken(scopes: string | string[], options?: GetTokenOptions | undefined): Promise<AccessToken | null> {
    ...
    if (token) console.log(`Token acquired for scope ${JSON.stringify(scopes)}: '${token.token.substring(0, 10)}...'`);
  ...
  async getManagementBearerToken(): Promise<string> {
    return await this.getToken("https://management.core.windows.net//user_impersonation").then((accessToken) => {
